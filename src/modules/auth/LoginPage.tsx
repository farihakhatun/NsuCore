...
export function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isRegister) {
        ...
        const res = await fetch('/identifier/v1/People/Signup', {
          method: 'POST',
        ...
        const projectKey = import.meta.env.VITE_X_BLOCKS_KEY;
        const res = await fetch('/idp/v1/Authentication/Token', {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'x-blocks-key': projectKey },
          credentials: 'include',
          body: formData,
        });
        ...
        const refreshToken = result?.refresh_token || result?.refreshToken || result?.data?.refresh_token;

        if (token) {
          // Store in authStore
          const authStore = useAuthStore.getState();
          authStore.setAccessToken(token);
          if (refreshToken && typeof authStore.setRefreshToken === 'function') {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg,#e8f4f8 0%,#d4eaf7 40%,#c8e6f5 100%)', fontFamily: 'Segoe UI,sans-serif', padding: '20px' }}>
      ...
      <div style={{ ...glass, padding: '48px 40px', width: '100%', maxWidth: '420px', position: 'relative', zIndex: 1, boxShadow: '0 25px 60px rgba(100,180,220,0.2)' }}>
        ...
        <form onSubmit={handleSubmit}>
          <h2 style={{ color: '#1a3a4a', fontSize: '20px', marginBottom: '24px', fontWeight: 600 }}>{isRegister ? 'Create Account' : 'Welcome Back'}</h2>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)}
                style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', color: 'rgba(20,60,80,0.5)', padding: 0, lineHeight: 1 }}>
                {showPassword ? '🙈' : '👁️'}
              </button>
            </div>
          </div>
